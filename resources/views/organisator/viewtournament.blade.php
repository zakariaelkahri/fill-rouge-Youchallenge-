...
<div class="bg-gray-800 rounded-xl shadow-lg p-6 mb-6">
    <div class="flex justify-between items-center mb-6">
        ...
        @if ($next_round == true && !session('lastRound') && $tournament->status == 'ongoing')
            <form action="{{route('organisator.start.tournament',['tournament'=>$tournament->id])}}" method="POST">
                @csrf
                                ...
                                <div class="flex-1 flex items-center">
                                    ...
                                    <div class="flex flex-col items-start flex-1">
                                        ...
                                        <div class="team-select cursor-pointer flex items-center p-2 rounded-lg transition-colors 
                                            ...
                                            <div class="font-medium text-white">
                                                {{ $match->status == 'finished' && $match->winner_id == $match->team1_id ? 'Match Winner' : 'Select as winner' }}
                                                @if($match->status == 'finished' && $match->winner_id == $match->team1_id)
                                                    <i class="fas fa-trophy text-yellow-400 ml-1"></i>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex items-center mx-4">
                                        <input type="number" min="0" name="matches[{{ $matchIndex }}][score_team1]" value="{{ $match->getResault($match->id)->score_team1 }}" 
                                            class="w-12 text-center bg-gray-700 text-white rounded-lg px-2 py-1 border border-gray-600 focus:border-blue-500 focus:ring focus:ring-blue-500/20 score-input {{ $match->winner_id == $match->team1_id ? 'border-green-500 bg-green-900/30 font-bold' : '' }}"
                                        ...
                                        <input type="number" min="0" name="matches[{{ $matchIndex }}][score_team2]" value="{{ $match->getResault($match->id)->score_team2 }}" 
                                            class="w-12 text-center bg-gray-700 text-white rounded-lg px-2 py-1 border border-gray-600 focus:border-blue-500 focus:ring focus:ring-blue-500/20 score-input {{ $match->winner_id == $match->team2_id ? 'border-green-500 bg-green-900/30 font-bold' : '' }}"
                                            {{  $match->status == 'finished' ? 'disabled' : '' }}>
                                    </div>                                                
                                    @else
                                    <div class="flex items-center mx-4">
                                    ...
                                    <div class="flex flex-col items-end flex-1">
                                        ...
                                        <div class="team-select cursor-pointer flex items-center p-2 rounded-lg transition-colors 
                                            ...
                                            <div class="font-medium text-white">
                                                {{ $match->status == 'finished' && $match->winner_id == $match->team2_id ? 'Match Winner' : 'Select as winner' }}
                                                @if($match->status == 'finished' && $match->winner_id == $match->team2_id)
                                                    <i class="fas fa-trophy text-yellow-400 ml-1"></i>
                                                @endif
                                            </div>
                            ...
                            @if($match->status == 'finished')
                                <div class="mt-3 text-center">
                                    <div class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-green-900/30 text-green-400 border border-green-600">
                                        ...
                                        Match completed - Winner: {{ $match->getTeamName($match->winner_team) }}
